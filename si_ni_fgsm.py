...
def graph(x, y, i, x_max, x_min, grad):
    ...
    beta_2 = 0.25
    beta_3 = 0.20
    print("b1 =", beta_1, "b2 =", beta_2,  "b3 =", beta_3,"梯度迭代版")

    x_nes_a = x + momentum * alpha * grad
    x_nes_b = x_nes_a + momentum * alpha * grad
    x_nes_c = x_nes_b + momentum * alpha * grad

######################----1----#############################################################
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(
            x_nes_a, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    ...
    noise_a = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)

    ######################----2----#############################################################

    
    # x_nes_b = x_nes_a + momentum * alpha * noise_a
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(
            x_nes_b, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    ...
    noise_b = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)
######################----3----#############################################################
    # x_nes_c = x_nes_a + momentum * alpha * noise_b
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(
            x_nes_c, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    ...
    noise_c = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)




    noise = momentum * grad + beta_1 * noise_a + beta_2 * noise_b + beta_3 * noise_c
