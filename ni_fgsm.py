...
#     beta_1 = 0.6
#     beta_2 = 0.4
#     print("b1 =", beta_1, "b2 =", beta_2)
#
#     x_nes_1 = x + momentum * alpha * grad
#     x_nes_2 = x_nes_1 + momentum * alpha * grad
#
#     #####################----1----#####################################
#     with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
#         logits_v3, end_points_v3 = inception_v3.inception_v3(
#             x_nes_1, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
#
#     pred = tf.argmax(end_points_v3['Predictions'], 1)
#     first_round = tf.cast(tf.equal(i, 0), tf.int64)
#     y = first_round * pred + (1 - first_round) * y
...
#     noise = tf.gradients(cross_entropy, x)[0]
#
#     noise_1 = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)
#     #####################----2----#####################################
#     with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
#         logits_v3, end_points_v3 = inception_v3.inception_v3(
...
#     noise = tf.gradients(cross_entropy, x)[0]
#
#     noise_2 = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)
#
#     #######################################################################
#
#
#     noise = momentum * grad + beta_1 * noise_1 + beta_2 * noise_2
#
#     x = x + alpha * tf.sign(noise)
#     x = tf.clip_by_value(x, x_min, x_max)
#     i = tf.add(i, 1)
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    beta_2 = 0.3
    beta_3 = 0.3
    print("b1 =", beta_1, "b2 =", beta_2,  "b3 =", beta_3)

    x_nes_1 = x + momentum * alpha * grad
    x_nes_2 = x_nes_1 + momentum * alpha * grad
    x_nes_3 = x_nes_2 + momentum * alpha * grad
    ...
    with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
        logits_v3, end_points_v3 = inception_v3.inception_v3(
            x_nes_3, num_classes=num_classes, is_training=False, reuse=tf.AUTO_REUSE)
    ...
    noise_3 = noise / tf.reduce_mean(tf.abs(noise), [1, 2, 3], keep_dims=True)

    #######################################################################

    noise = momentum * grad + beta_1 * noise_1 + beta_2 * noise_2 + beta_3 * noise_3
...
if __name__ == '__main__':
    ...
    # docker run - it - -name KEVIN anaconda /bin/bash
